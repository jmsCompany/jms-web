...
//            $(t).trigger('change');
	...
	//在插件中使用Select对象
	$.fn.selectautofill1 = function(path,options,callback) {
		var t = this;
		$.JMSClient(path,options,function(data){
			t.html("");
			$(data).each(function(i, val){
				var a = i+1;
				t.append($('<option value='+val.id+'>'+a+'_'+val.name+'</option>'));
			});
			$(t).trigger('change');
	...
	//在插件中使用Select2对象
	$.fn.selectautofill2 = function(path,options,callback) {
		var t = this;
		$.JMSClient(path,options,function(data){
			t.html("");
			$(data).each(function(i, val){
				t.append($('<option value='+val.sid+'>'+val.name+'</option>'));
			});
			$(t).trigger('change');
	...
	//在插件中使用Select对象
	$.fn.selectautofill3 = function(path,optId,options,callback) {
		var t = this;
		$.JMSClient(path,options,function(data){
			t.html("");
			$(data).each(function(i, val){
...
//			$(t).trigger('change');
	...
	$.saveUserPassword1=function(userPassword,jmstoken,callback){
		$.JMSClient('user/updateUserPassword',{'type':'POST',headers:{'JMS-TOKEN':jmstoken},data:userPassword},callback);
	};
	//显示用户
...
/*
	...
	//公司流转--拒绝接收
	$.refuseSmtf=function(mtId,jmstoken,callback){
		$.JMSClient('s/refuseSmtf',{headers:{'JMS-TOKEN':jmstoken},data:{mtId:mtId}},callback);
	};
	//删除物料仓库关系
	...
	//读取自动编码设置
	$.getWSNumSet=function(jmstoken,callback){
			$.JMSClient('s/getWSNumSet',{'type':'GET',headers:{'JMS-TOKEN':jmstoken}},callback)
	};
	//保存自动编码设置
	$.saveWSNumSet=function(numSet,jmstoken,callback){
			$.JMSClient('s/saveWSNumSet',{'type':'POST',headers:{'JMS-TOKEN':jmstoken},data:numSet},callback)
	};
    //保存检查设置 未实现
...
//}
	//合作公司列表
	$.coCompanise=function(jmstoken,callback){
          $.JMSClient('s/myCoCompanies',{headers:{'JMS-TOKEN':jmstoken}},callback);
	};

	//点击共享
	$.shareBom=function(param,jmstoken,callback){
          $.JMSClient('p/shareBom',{'type':'POST',headers:{'JMS-TOKEN':jmstoken},data:param},callback);
	};

	$.matbomListInfo = function(productId,jmstoken,callback){
		$.JMSClient('p/findBomByProductId',{headers:{'JMS-TOKEN':jmstoken},data:{productId:productId}},callback);
	};

    $.matbomListInfo1 = function(productId,comCompanyId,jmstoken,callback){
		$.JMSClient('p/findBomByProductIdAndComComannyId',{headers:{'JMS-TOKEN':jmstoken},data:{productId:productId,comCompanyId:comCompanyId}},callback);
	};

	$.getReceiveSendSet =function(jmstoken,callback){
		$.JMSClient('s/getOverfulfill',{'type':'GET',headers:{'JMS-TOKEN':jmstoken}},callback);
	};
	$.saveReceiveSendSet =function(RSsent,jmstoken,callback){
		$.JMSClient('s/saveOverfulfill',{'type':'POST',headers:{'JMS-TOKEN':jmstoken},data:RSsent},callback);
	};
	$.updateComCom =function(data,jmstoken,callback){
		$.JMSClient('s/updateComCom',{'type':'POST',headers:{'JMS-TOKEN':jmstoken},data:data},callback);
	};
	$.getMaterialPriceList =function(id,jmstoken,callback){
		$.JMSClient('s/mrp/cocompanyPriceList',{'type':'GET',headers:{'JMS-TOKEN':jmstoken},data:{materialId:id}},callback);
	};
	$.saveMaterialPriceList =function(data,jmstoken,callback){
		$.JMSClient('s/mrp/savecocompanyPriceList',{'type':'POST',headers:{'JMS-TOKEN':jmstoken},data:data},callback);
	};
	$.saveBuyMethod  = function(data,jmstoken,callback){
		$.JMSClient('s/mrp/saveBuyMethod ',{'type':'POST',headers:{'JMS-TOKEN':jmstoken},data:data},callback);
	};
	$.deleteMatPrice =function(data,jmstoken,callback){
